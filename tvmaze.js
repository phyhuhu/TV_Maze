...
async function searchShows(query) {
  ...
  let movies=res.data.map(x => {
    return {
      id: x.show.id,
      name: x.show.name,
      summary: x.show.summary,
      image: x.show.image ? x.show.image.medium : "#"
    }
  });
...
$("#shows-list").on("click", async function(evt) {
  ...
  const listEpisodes=$("#episodes-list");
  listEpisodes.empty();

  episodes.map(x => {
    let episode=`<li> ${x.name} (season ${x.season}, episode ${x.number})</li>`;
